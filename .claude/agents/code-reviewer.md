...
and a response costs the model's latency whatever the commands return.
...
later response re-reads what a call returned, so bytes taken in early are paid for by every
response after them, where the merged response is saved once. List what a stage needs, then run that list. A `Read` cannot join a chain, so
...
- **security**: each side of a boundary validates what reaches it, whatever the other side
  claims to have checked. This app ships to the browser whole, so its boundaries are the
  API response, the URL, and anything a person types: a response field a gateway hands on
  without decoding it, a value out of a response or a route parameter that reaches `href`,
  `src`, `redirect()` or `dangerouslySetInnerHTML` unchecked, a `VITE_`-prefixed env var
  holding a credential, since every one of them is readable in the built bundle. Validation
  written here bounds what this app does with a response and nothing the API accepts, so a
  finding that needs the API to check something is reported as that. Severity here is
  critical or major.
- **cleanup**: duplication, dead code, needless complexity, obvious performance problems,
  drift from surrounding conventions. `pnpm check` reports formatting, import order and
...
- `external-behaviour`: the candidate turns on how a CLI flag, a config key, or a library
  ...
  states it, such as AGENTS.md on what `pnpm check` runs, is such a source and settles the
...
A whole-file open costs more bytes than a window, and every later response pays those bytes
again, so reserve a whole-file `Read` for a file you need end to end.
...
`pnpm test <path> --coverage.enabled=false`: `pnpm test` alone carries `--coverage` and this
repository's per-file branch threshold, so a filtered run prints a threshold error for every
file the filter never loaded and exits 1 on a passing test.
...
## Abstained
- src/lib/baz.ts:71 — `vp fmt` may rewrite this template literal · external-behaviour ·
  re-read src/lib/baz.ts:64-78 · neither the briefing nor any file here states what that
  formatter does to it
