...
| 15 | Object storage directory listing is private | Check the public access settings of the object storage bucket |
...
| 22 | Periodic DB backups are enabled | Check the database's backup settings |
...
| # | Item | How to verify |
|---|------|---------------|
| 35 | Every page has an appropriate `<title>` tag | Read the title each route sets, then confirm it in the browser tab |
...
| 40 | Key pages (top page, etc.) have a meta description | Read the meta tags each route sets |
...
| # | Item | How to verify |
|---|------|---------------|
| 56 | Images have appropriate `alt` attributes | Grep `src/` for `<img` and read every `alt` |
...
| # | Item | How to verify |
|---|------|---------------|
| 64 | The UI does not break at phone / tablet sizes | Resize the browser window to phone and tablet widths by hand |
...
| 70 | `<html lang="...">` is set | Read `index.html` |
