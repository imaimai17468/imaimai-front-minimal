...
A popover, dropdown, or tooltip scales from its trigger. A Radix primitive
publishes that point as a CSS variable, which a component reads through
Tailwind's `origin-(--radix-<part>-content-transform-origin)`. A modal keeps
...
- Build on primitives and restyle what you take. Taking a prebuilt block's
  behavior costs nothing, and taking its styling costs the identity. This
  repository ships no primitive: `src/components/ui/` is where a generated one
  (shadcn CLI output) goes, and `README.md` says what adding it needs.
